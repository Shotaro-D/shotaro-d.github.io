import {
  ...
  wheelInteraction,
  wheelScaleFactor,
} from "./manual_geometry.js";
import { decodeTiff } from "./tiff_decoder.js";
...
async function startManualApp() {
  ...
  try {
    const meshPayload = await loadStaticMeshes();
...
async function loadStaticMeshes() {
  const response = await fetch("/static/manual_meshes.json", { credentials: "same-origin" });
  let payload = null;
  try {
    payload = await response.json();
  } catch (_) {
    // Use the status-specific error below when a deploy serves malformed data.
  }
  if (!response.ok || !payload) {
    throw new Error(`3Dモデル情報を読み込めません（${response.status}）。`);
  }
  return payload;
}
...
function buildLocalCalibration(image, sidecar, tiffMetadata) {
  ...
  const detected = detectScaleMarker(image, markerLength, metadataPixelSize);
...
function automaticallyDetectBmpScaleBar() {
  const marker = detectScaleMarker(state.image, null, null).marker;
