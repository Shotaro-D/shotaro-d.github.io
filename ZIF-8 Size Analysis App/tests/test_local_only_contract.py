...
def test_browser_reads_and_saves_user_files_without_uploading_them():
    ...
    assert "LOCAL_SESSION_STORAGE_TARGET_BYTES" in manual_javascript
    assert "evictOldLocalSessions" in manual_javascript
    ...
    assert 'id="changeProjectButton"' in html
...
def test_browser_analysis_code_has_no_network_primitives_or_virtual_api_routes():
    analysis_modules = [
        PROJECT_ROOT / "static" / name
        for name in (
            "manual.js",
            "manual_geometry.js",
            "manual_local_paths.js",
            "manual_scale.js",
            "manual_state.js",
            "manual_statistics.js",
            "tiff_decoder.js",
        )
    ]
    source = "\n".join(path.read_text(encoding="utf-8") for path in analysis_modules)
    for primitive in ("XMLHttpRequest", "WebSocket", "EventSource", "sendBeacon", "FormData"):
        assert primitive not in source
    assert "/api/manual/" not in source
    assert source.count("fetch(") == 1
    assert 'fetch("/static/manual_meshes.json", { credentials: "same-origin" })' in source

    auth_javascript = (PROJECT_ROOT / "static" / "app.js").read_text(encoding="utf-8")
    for forbidden in ("XMLHttpRequest", "WebSocket", "EventSource", "sendBeacon"):
        assert forbidden not in auth_javascript
    assert "new FormData(form)" in auth_javascript
