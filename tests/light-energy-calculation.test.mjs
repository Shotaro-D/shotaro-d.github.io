...
const runSuite = (pageUrl) => {
  const html = readFileSync(new URL(pageUrl, import.meta.url), 'utf8');
  ...
  console.log(
    `PASS (${pageUrl}): ${standardPathCount} standard paths, ${edgeCases.length} extreme paths, ` +
    `maximum relative error ${maximumRelativeError}; defaults, rounding warning, validation, and decimal-only output verified.`
  );

  return calculatorScript;
};

const jaCalculatorScript = runSuite('../light-energy-calculation.html');
const enCalculatorScript = runSuite('../en/light-energy-calculation.html');

const stripErrorMessages = (script) => script.replace(/new Error\('[^']*'\)/g, "new Error('')");

assert.equal(
  stripErrorMessages(jaCalculatorScript),
  stripErrorMessages(enCalculatorScript),
  'The Japanese and English calculator scripts differ beyond the Error() message strings.'
);

console.log('PASS: the Japanese and English calculator scripts are identical apart from the Error() message strings.');
