...
PAGES = [
    ROOT / "light-energy-calculation.html",
    ROOT / "en" / "light-energy-calculation.html",
]
...
def javascript_constant(html, name):
...
def check(page: Path):
    ...
    c = javascript_constant(html, "LIGHT_SPEED_M_S")
    h = javascript_constant(html, "PLANCK_J_S")
    ev_j = javascript_constant(html, "EV_J")
    hc_j_m = c * h
    hc_ev_m = hc_j_m / ev_j
    ...
    print(
        f"PASS ({page.relative_to(ROOT)}): exact SI constants, unit inventories/defaults, "
        "four benchmark values, and 1134 Decimal reference paths verified."
    )
